...
namespace eShop.Models
{
    ...
    public class PackingWorkOrderViewModel
    {
        [Key]
        [DatabaseGenerated(DatabaseGeneratedOption.Identity)]
        public int Id { get; set; }
    ...
    public class PackingWorkOrderFinishedGoodSlipViewModel
    {
        [Key]
        [DatabaseGenerated(DatabaseGeneratedOption.Identity)]
        public int Id { get; set; }
    ...
    // All outsanding Spk Packing ke Penyelesaian Barang Jadi
    public class OutsandingPackingWorkOrderFinishedGoodSlipDatalist : MvcDatalist<PackingWorkOrderFinishedGoodSlipViewModel>
    {
        private DbContext Context { get; }

        public OutsandingPackingWorkOrderFinishedGoodSlipDatalist(DbContext context)
        {
            Context = context;

            GetLabel = (model) => model.Code;
        }
        public OutsandingPackingWorkOrderFinishedGoodSlipDatalist()
        {
            Url = "/DatalistFilters/AllOutsandingPackingWorkOrderFinishedGoodSlip";
        ...
        public override IQueryable<PackingWorkOrderFinishedGoodSlipViewModel> GetModels()
        {
            return Context.Set<PackingWorkOrder>()
                .Where(x => !Context.Set<FinishedGoodSlip>().Where(p => p.Active == true && p.PackingWorkOrderId == x.Id).Any())
                .Select(x => new PackingWorkOrderFinishedGoodSlipViewModel
                {
                    Id = x.Id,
    ...
    // All outsanding Spk Packing ke Pengambilan Bahan Baku
